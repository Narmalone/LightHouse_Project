...
public class UiGridRenderer : MaskableGraphic
{
    ...
    public bool IsCenter = true;  // Nouveau booléen pour gérer le centrage
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        // Si IsCenter est false, on génère à partir de l'ancre (coin supérieur gauche)
        if (IsCenter)
        {
            xOffset = -width / 2f;
            yOffset = -height / 2f;
        ...
        for (int y = 0; y < gridSize.y; y++)
        {
            ...
            {
                DrawCell(x, y, count, vh, xOffset, yOffset);
                count++;
            }
        }
    }

    private void DrawCell(int x, int y, int index, VertexHelper vh, float xOffset, float yOffset)
    {
        float xPos = cellWidth * x + xOffset;
        float yPos = cellHeight * y + yOffset;
