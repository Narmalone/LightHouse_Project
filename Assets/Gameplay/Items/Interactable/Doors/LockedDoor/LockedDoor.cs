...
namespace LightHouse.Items.Interactable
{
    public class InteractionContext
    {
        ...
        public LocalizedString ActionWrapper;
    }
    ...
    public class LockedDoor : IDUseItemTracker, IDoor
    {
        ...
        protected override async void UpdateInteractionText()
        {
            ...
            if (!_isUnLocked)
            {
                ...
                {
                    var keyOp = _neededKey.GetLocalizedStringAsync();
                    await keyOp.Task;
        ...
        private async Task<InteractionContext> BuildInteractionContext()
        {
            ...
            if (!_isUnLocked)
            {
                if (!_hasKeyInInventory)
                {
                    var keyOp = _neededKey.GetLocalizedStringAsync();
                    await keyOp.Task;
                    context.Text = _missingItemInInventoryTxt;
                    context.Arguments = new object[] { new { key = keyOp.Result } };
                }
                else if (_hasKeyInInventory && !_hasKeyOnHands)
                {
                    var keyOp = _neededKey.GetLocalizedStringAsync();
                    await keyOp.Task;
                    context.Text = _needItemOnHandsTxt;
                    context.Arguments = new object[] { new { key = keyOp.Result } };
                }
                else
                {
                    context.Text = _unlockText;
                    context.Bind = InputManager.InteractInInventory_Bind_Name;
                    context.ActionWrapper = _holdToAction;
                }
            }
            else
            {
                context.Text = _isOpen ? _closeText : _openText;
                context.Bind = InputManager.Interact_Bind_Name;
                context.ActionWrapper = _pressToAction;
            }
