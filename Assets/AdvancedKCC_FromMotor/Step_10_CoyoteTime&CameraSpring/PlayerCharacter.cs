            ...
            if (_motor.GroundingStatus.IsStableOnGround)
            {
                ...
                //Start Sliding
                {
                    ...
                    if(isMoving && isCrouching && (wasStanding || wasInAir))
                    {
                        ...
                        if (wasInAir)
                        {
                            currentVelocity = Vector3.ProjectOnPlane(vector: _lastState.Velocity, planeNormal: _motor.GroundingStatus.GroundNormal);
                        }

                        var effectiveSlideSpeed = _slideStartSpeed;
                        if(!_lastState.Grounded && !_requestedCrouchInAir)
                        {
                            effectiveSlideSpeed = 0f;
                            _requestedCrouchInAir = false;
                        }
                        float slideSpeed = Mathf.Max(effectiveSlideSpeed, currentVelocity.magnitude);
                ...
                else
                {
                    ...
                    //Steer
                    {
                        ...
                        Vector3 steerForce = (targetVelocity - steerVelocity) * _slideSteerAcceleration * deltaTime;
