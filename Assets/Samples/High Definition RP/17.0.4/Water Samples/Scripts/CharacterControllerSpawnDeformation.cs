...
public class CharacterControllerSpawnDeformation : MonoBehaviour
{
    ...
    void Update()
    {
        if(IsControllerMoving())
        {
            ...
            {       
                if(WaterSamplePoolManager.Instances[WaterSamplePoolManager.InstanceType.Deformer] != null)
                {
                    GameObject deformer = WaterSamplePoolManager.Instances[WaterSamplePoolManager.InstanceType.Deformer].getNextAvailable();
                    if(deformer != null)
                    {
