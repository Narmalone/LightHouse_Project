#ifndef WATER_DECAL_DEFORMED_H
#define WATER_DECAL_DEFORMED_H
...
void EvaluateShoreWaveDecal_float(float2 uv, float waveLength, uint skippedWaves, float2 waveBlend,
    float waveOffset, float2 breakingRange, float2 deepFoamRange, float deformStrength,
    out float amplitude, out float2 foam)
{
    // Compute the overall position (takes into account wave speed and wave length)
    // float t = ((uv.x * 2 - 1) - waveOffset) / waveLength;

    // --- Déformation vague ---
    float variation = sin((uv.x + uv.y) * 12.3 + waveOffset) * 0.5 + 0.5;
    float totalDeform = variation * deformStrength;

    // Compute the overall position (takes into account wave speed and wave length) + deformation
    float t = (((uv.x * 2 - 1) - waveOffset) / waveLength) + totalDeform;
