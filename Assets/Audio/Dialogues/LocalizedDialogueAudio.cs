...
namespace LightHouse.Audio
{
    ...
    public class LocalizedDialogueAudio : ScriptableObject
    {
        ...
        public bool EnableAutoFallbackDuration = false;
        public float FallbackDuration = 3f;
        public float AdditionalDurationOnceLetterAppeared = 1f;
        ...
        public float GetDisplayDuration()
        {
            ...
            {
                float tempTotalDuration = 0f;
                foreach(char c in CurrentSubtitleText)
                {
                    tempTotalDuration += CharDelay;
                }
                FallbackDuration = tempTotalDuration + 0.5f;
            }
            return CurrentAudioClip != null ? CurrentAudioClip.length + AdditionalDurationOnceLetterAppeared : FallbackDuration;
        }
